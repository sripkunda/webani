...
import { WanimObjectAnimation } from "./animations";
import 'mathjax/es5/tex-svg';

export const triangulate = (points, holes) => {
    return earcut(points, holes);
};

export const windingOrderClockwise = (points) => {
    let sum = 0;
...
export const lineFrom = (startingX, startingY, endX, endY, t) => {
    const x = WanimObjectAnimation.lerp(startingX, endX, t);
    const y = WanimObjectAnimation.lerp(startingY, endY, t);
    return [x, y];
};

export const quadraticBezier = (startingX, startingY, controlX, controlY, endX, endY, t) => {
    const startPointX = WanimObjectAnimation.lerp(startingX, controlX, t);
    const endPointX = WanimObjectAnimation.lerp(controlX, endX, t);
    const startPointY = WanimObjectAnimation.lerp(startingY, controlY, t);
    const endPointY = WanimObjectAnimation.lerp(controlY, endY, t);
    return lineFrom(startPointX, startPointY, endPointX, endPointY, t);
};

export const cubicBezier = (startingX, startingY, startControlX, startControlY, endControlX, endControlY, endX, endY, t) => {
...
export const svgPathToPoints = (position, pathCommands, reflect = false, holesCW = false) => {
    let paths = [];
    let currentPoints = [];
    let startingX;
    let startingY;
    let prevControlPointX;
    let prevControlPointY;
    let minX;
    let minY;
    let maxX;
    let maxY;

    const performInterpolation = (interpolationFunction, ...args) => {
        let t = 0;
        while (t < 1) {
    ...
    for (let commandList of pathCommands) {
        for (let i in commandList.commands) {
            ...
            const getReflectedControl = () => {
                ...
                const prevCommand = commandList.commands[i - 1];
                if (prevCommand) {
            ...
            switch (command.type) {
                ...
                case 'q':
                    ...
                    break;
                case 'T':
                    let controlPoint = getReflectedControl();
            ...
            if (minX === undefined || minX > startingX) minX = startingX;
            if (minY === undefined || minY > startingY) minY = startingY;
            if (maxX === undefined || maxX < startingX) maxX = startingX;
            if (maxY === undefined || maxY < startingY) maxY = startingY;
        }
    }
...
export const parsePathData = (pathData) => {
    const commands = [];
...
export const parseLatexString = (string) => {
...
export const textToPoints = (string, position, fontSize) => {
    string = String(string);
    let pathCommands = [];
    string = parseLatexString(string);
    const parser = new DOMParser();
    const svg = parser.parseFromString(MathJax.startup.adaptor.innerHTML(MathJax.tex2svg(string, {
        display: true,
    })), "image/svg+xml");
    ...
    for (let definition of pathDefinitions) {
        ...
        pathCommands.push(...pathInstances.map(instance => {
            let transformation = {
                scale: [1, 1],
                translation: [0, 0]
            };
...
export async function executeInParallel(funct, args, threadCount = Math.min(navigator.hardwareConcurrency, 2)) {
    const result = [];
    while (args.length) {
...
export function pointInPolygon(polygonPoints, point) {
    let [px, py] = point;
    let windingNumber = 0;
...
export function pointsInPolygon(polygonPoints, points) {
