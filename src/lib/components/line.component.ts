...
export class LineComponent extends Component {
    objectConstructor(position: Vector2, length: number, angle = 0, thickness = 2, color = Colors.WHITE, opacity = 1): ObjectLike {
        return Rectangle(position, length, Math.max(2, thickness), color, opacity).SetCenterPosition([position[0] + length / 2, position[1], 0]).SetRotation(angle, [position[0], position[1], 0])
    }
}
